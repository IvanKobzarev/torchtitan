...
class GraphGradientState:
    """Persistent optimizer-visible gradient buffers for a traced model."""
    ...
    def create(
        ...
        cls._validate_optimizer_membership(parameters, optimizers)
    ...
    def validate_optimizers(
        ...
        self._validate_optimizer_membership(self.parameters, optimizers)
...
def _accumulation_leaf_offsets(
    fqn: str,
    buffer_layout: SubclassLayout | None,
    gradient_layout: SubclassLayout | None,
) -> tuple[int, ...]:
    return _graph_state_leaf_offsets(fqn, buffer_layout, gradient_layout)[0]
...
def _validate_gradient_leaf_mapping(
    ...
    if (fqn, leaf_offset) not in gradient.meta.get(_GRAPH_STATE_OUTPUT_META, ()):
        raise ValueError(
            "Graph pass changed the explicit gradient-output mapping for " f"{fqn!r}"
        )


def validate_graph_gradient_output_mapping(
    gm: torch.fx.GraphModule,
    traced_result: TracedResult,
) -> None:
    """Verify that graph passes preserved the explicit gradient output mapping."""
    ...
    if len(output_leaves) != expected_num_output_leaves:
        raise ValueError(
            "Graph output metadata changed: "
    ...
    for fqn, output_index in zip(
        ...
        if not 0 <= output_index < len(output_ranges):
            raise ValueError(
                f"Gradient output index {output_index} for {fqn!r} is outside "
        ...
        for leaf_index, flat_index in enumerate(output_ranges[output_index]):
            ...
            if not isinstance(leaf, torch.fx.Node):
                raise ValueError(
                    f"Gradient output {output_index} for {fqn!r} is not a tensor"
                )
            if (fqn, leaf_index) not in leaf.meta.get(_GRAPH_STATE_OUTPUT_META, ()):
                raise ValueError(
                    "Graph changed the explicit gradient-output mapping for " f"{fqn!r}"
                )
...
def finalize_graph_gradient_accumulation(
    ...
    *,
    ...
    """Accumulate returned gradients into graph state and return only loss."""
    del example_inputs
    if traced_result.grad_sink_active:
        return gm
    if not traced_result.graph_state_fqns:
        raise ValueError("Graph gradient accumulation requires non-empty graph state")
    if not (
        len(traced_result.graph_state_fqns)
        == len(traced_result.graph_state_input_indices)
        == len(traced_result.graph_state_output_indices)
    ):
        raise ValueError(
            "Graph gradient state requires one input and output mapping per buffer"
        )
    ...
    gradient_output_index_set = set(traced_result.graph_state_output_indices)
    if len(gradient_output_index_set) != len(traced_result.graph_state_output_indices):
        raise ValueError("Graph gradient output indices must be unique")
    loss_output_indices = [
        output_index
    ...
    with gm.graph.inserting_before(output):
        for state_index, (fqn, buffer_indices, gradient_output_index) in enumerate(
            ...
            if not 0 <= gradient_output_index < len(output_ranges):
                raise ValueError(
                    f"Gradient output index {gradient_output_index} for {fqn!r} "
                    f"is outside the {len(output_ranges)} traced outputs"
                )
    ...
    traced_result.grad_sink_active = True
    return gm
