...
class CodaPattern:
    ...
    best_configs_by_shape: dict[
        int, dict[tuple[int, int, int], tuple[dict[str, Any], ...]]
...
def register_coda_pattern(
    *,
    ...
    """Register one matcher and optional device- and shape-specific configs."""
    ...
    def register(matcher: CodaMatcher) -> CodaMatcher:
        ...
        _CODA_PATTERNS[name] = CodaPattern(
            ...
            {} if best_configs_by_shape is None else best_configs_by_shape,
        )
        return matcher
...
def _best_configs(
    ...
    if device_capacity == 11:
        device_capacity = 10
    registered = _CODA_PATTERNS[pattern]
    if root is not None:
...
def _kernel_options(
    ...
    *,
    ...
    configs = () if autotune else _best_configs(pattern, root)
    if configs:
    ...
    with gm.graph.inserting_after(body_attr):
        fused = gm.graph.call_function(
            ...
            args=(
                ...
                {},
                _kernel_options(pattern, root=root, autotune=autotune),
            ),
        )
    ...
    *,
    ...
    fused_first = _call_after(
        ...
        (
            ...
            tuple(external_inputs),
            {},
            _kernel_options(pattern, root=first_path.root, autotune=autotune),
        ),
    )
...
def _insert_forward_rmsnorm_fusion(
    ...
    *,
    ...
    if benchmark_regions is not None:
        ...
        def rewrite(
            ...
            _insert_forward_rmsnorm_fusion(
                ...
                pattern=pattern,
                autotune=autotune,
                flex_gemm_options=flex_gemm_options,
            )
    ...
    region = _CODA_INDUCTOR_REGION
    options = (
        _kernel_options(pattern, root=path.root, autotune=autotune)
        if flex_gemm_options is None
        else flex_gemm_options
    )
...
@register_coda_pattern(
    best_configs_by_shape={
        10: {
            (8192, 2048, 10944): (
                _quack_config(256, 224, dynamic=True, cluster_n=2),
                _quack_config(256, 224, dynamic=True),
            ),
        }
    },
)
def F_swiglu(  # noqa: N802
    ...
    for output in list(gm.graph.nodes):
        ...
        if match is None or not _claim_coda_match(pattern, output, selection):
            continue
...
@register_coda_pattern(
    ...
    best_configs_by_shape={
        10: {
            (8192, 2048, 10944): (_quack_config(256, 224, dynamic=True),),
        }
    },
)
def B_swiglu_backward_activation(  # noqa: N802
    ...
    for node in list(gm.graph.nodes):
        ...
        if not any(
            ...
            continue
        if not validate_partition(_ordered_nodes(gm, descendants)):
            continue
        if not _claim_coda_match(pattern, node, selection):
            continue
...
@register_coda_pattern(
    best_configs_by_shape={
        10: {
            (2048, 8192, 10944): (_quack_config(256, 256, dynamic=True, cluster_n=2),),
            (10944, 8192, 2048): (
                _quack_config(256, 192, dynamic=False, swap_ab=True),
            ),
            (102400, 1024, 2048): (
                _quack_config(256, 256, dynamic=False, swap_ab=True),
            ),
        }
    },
)
...
def _apply_coda_pattern(
    ...
    *,
    ...
    pattern.matcher(gm, counts, benchmark_regions, selection, autotune)
    if benchmark_regions is not None:
        return gm
...
def _configured_coda_pass(
    ...
    *,
    ...
    def apply(
        ...
        elif pattern.best_configs_by_shape and configs:
            logger.info(
                f"CODA {pattern.name} using shape-specific FlexGEMM configs "
                f"with pinned fallback: {configs}"
            )
        elif pattern.best_configs_by_shape:
            logger.info(f"CODA {pattern.name} using shape-specific FlexGEMM configs")
        elif configs:
