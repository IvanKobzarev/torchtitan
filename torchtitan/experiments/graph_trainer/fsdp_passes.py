...
def _validate_transformer_block_bucket_counts(
    ...
    *,
    ...
    expect_backward_all_gathers: bool,
) -> None:
    missing_expected = [
        layer_id
    ...
    for layer_id in sorted(local_layer_ids):
        ...
        expected_by_kind = {
            "fwd_ag": expected,
            "bwd_ag": expected if expect_backward_all_gathers else 0,
            "bwd_rs": expected,
        }
        mismatches = {
            kind: count
            for kind, count in actual.items()
            if count != expected_by_kind[kind]
        }
        if mismatches:
            errors.append(
                f"layer {layer_id}: expected "
                f"fwd_ag={expected_by_kind['fwd_ag']} "
                f"bwd_ag={expected_by_kind['bwd_ag']} "
...
def schedule_fsdp_comms_to_dense_regions_pass(
    ...
    *,
    ...
    expect_backward_all_gathers: bool = True,
    strict: bool = False,
) -> torch.fx.GraphModule:
    ...
    if transformer_bucket_counts_by_layer is not None:
        _validate_transformer_block_bucket_counts(
            ...
            expect_backward_all_gathers=expect_backward_all_gathers,
        )
